...
router.post('/join-club', async function (req, res, next) {
  ...
  var check = member_details.findOneAndUpdate(
    { club_id: ObjectId(club_id), member_list: { $nin: { user_object } } },
    {
    ...
    if (err) {
      ...
      async () => {
        var update = user_details.findOneAndUpdate(
          { _id: ObjectId(user_id), clubs: { $nin: { club_object } } },
          {
        ...
        await update.exec((err, data2) => {
          if (err) {
            ...
            var error = {
              is_error: true,
              message:
                'wrong club details or you are already part of this club',
            };
            return res.status(501).send(error);
          } else {
            var finaldata = {
...
router.post('/leave-club', async function (req, res, next) {
  ...
  var check = member_details.findOneAndUpdate(
    { club_id: ObjectId(club_id), member_list: { $in: { user_object } } },
    {
